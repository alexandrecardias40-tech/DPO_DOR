...
import { getPiValue, getReferenceYearFromRows } from "@/lib/budget";
...
export default function Home() {
  ...
  const statusFilters = useMemo(
    () => [
      ...
      {
        value: "vencendo",
        label: referenceYear ? `Vencendo (${referenceYear})` : "Vencendo",
      },
  ...
  const hasActiveFilters = Boolean(selectedDescription.length > 0 || selectedPi.length > 0);
  ...
  const filteredData = useMemo(() => {
    if (!allData) return [];
    if (selectedDescription.length === 0 && selectedPi.length === 0) return allData; // Show all data for table, but totals will be zero
    return allData.filter((item: any) => {
      const itemDescription = (item.Despesa || "").replace(/^-+\s*/, "");
      const itemPi = getPiValue(item);
      const descriptionMatch = selectedDescription.length === 0 || selectedDescription.includes(itemDescription);
      const piMatch = selectedPi.length === 0 || selectedPi.includes(itemPi);
      return descriptionMatch && piMatch;
    });
  }, [allData, selectedDescription, selectedPi]);
  ...
  const filteredTimeline = useMemo<any[]>(() => {
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-start flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-slate-600 mt-1">
              Análise de despesas e execução orçamentária
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <Card className="bg-white border-l-4 border-l-purple-500">
            <CardHeader className="py-1 px-3">
              <CardTitle className="text-xs font-semibold text-slate-600 tracking-wide uppercase">Valor Contrato Média mensal</CardTitle>
            </CardHeader>
            <CardContent className="pt-0 pb-3 px-3">
          ...
          <Card className="bg-white border-l-4 border-l-yellow-500">
            <CardHeader className="py-1 px-3">
              <CardTitle className="text-xs font-semibold text-slate-600 tracking-wide uppercase">
                Saldo Empenhado {referenceYear || 'Ano'}
              </CardTitle>
            </CardHeader>
            <CardContent className="pt-0 pb-3 px-3">
          ...
          <Card className="bg-white border border-slate-200 shadow-sm">
            <CardHeader className="pb-2 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="flex flex-wrap items-end gap-3 text-xs text-slate-600">
                <label className="flex flex-col gap-1 font-semibold text-slate-700 text-center">
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      ...
                      >
                        {selectedDescription.length > 0
                          ? `${selectedDescription.length} selecionado(s)`
                          : "Selecione descrições..."}
                ...
                <label className="flex flex-col gap-1 font-semibold text-slate-700 text-center">
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      ...
                      >
                        {selectedPi.length > 0
                          ? `${selectedPi.length} selecionado(s)`
                          : "Selecione PIs..."}
                        <ChevronDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                      </Button>
                    </PopoverTrigger>
              ...
              <table className="w-full table-fixed border-collapse text-[11px] md:text-xs">
                <thead>
                  <tr className="text-[10px] uppercase tracking-wide text-slate-500">
                    ...
                    <th className="px-2 py-2 text-center w-[10%]">Total Anual</th>
                    ...
                    filteredTimeline.map((item: any) => (
                      <tr key={item.id} className="border-b border-slate-100 text-slate-900 align-top">
                        ...
                        <td className="px-2 py-3 text-center whitespace-nowrap">
                          <div className="text-xs font-semibold text-slate-400 mb-1">Total Anual</div>
