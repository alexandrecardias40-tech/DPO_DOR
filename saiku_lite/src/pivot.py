...
def build_pivot(
    ...
    numeric_aggregators = {"sum", "money_sum", "avg", "min", "max"}
    working_frame = frame
    if aggregator in numeric_aggregators:
        working_frame = frame.copy()
        for selected in measures:
            working_frame[selected] = pd.to_numeric(working_frame[selected], errors="coerce")
    ...
    if rows and columns:
        ...
        grouped = working_frame.groupby(rows, dropna=False, sort=False)[pivot_values].agg(aggfunc)
    else:  # columns only
        grouped = working_frame.groupby(columns, dropna=False, sort=False)[pivot_values].agg(aggfunc)
